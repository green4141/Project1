...
public interface UserMapper {
	...
	@Select({
	  ...
	  "SELECT * FROM user",
	  "<where>",
	  "  <if test='id != null and id != \"\"'> id LIKE CONCAT('%', #{id}, '%') ESCAPE '\\\\' </if>",
	  "  <if test='name != null and name != \"\"'> name LIKE CONCAT('%', #{name}, '%') ESCAPE '\\\\' </if>",
	  "  <if test='username != null and username != \"\"'> username LIKE CONCAT('%', #{username}, '%') ESCAPE '\\\\' </if>",
